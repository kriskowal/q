...
describe("after", function () {
    ...
    it("continues primary chain once secondary promise fulfills", function () {
        var primaryResolved = Q(10);
        var secondaryDeferred = Q.defer();

        promise = primaryResolved
            .after(secondaryDeferred.promise)
            .then(function(value) {
                expect(secondaryDeferred.promise.inspect()).toEqual({
                    state: "fulfilled",
                    value: 5
                });
                return value;
        ...
        secondaryDeferred.resolve(5);
        return promise;
    });

    it("returns primary promise's value once secondary promise fulfills", function () {
        promise
            .then(function(value) {
                expect(value).toEqual(10);
            }, function() {
    ...
    //
    it("returns secondary rejection despite primary fulfillment", function () {
        var primaryResolved = Q(4);
        var secondaryRejected = Q.reject("secondary rejected");

        promise = primaryResolved
            .after(secondaryRejected)
            .then(function() {
                expect(true).toBe(false);
            }, function(reason) {
                expect(reason).toEqual("secondary rejected");
            });

        return promise;
    });

    it("doesn't return rejected primary as fulfilled when secondary fulfills", function () {
        var primaryRejected = Q.reject("primary rejected");
        var secondaryResolved = Q(5);

        promise = primaryRejected
            .after(secondaryResolved)
            .then(function() {
                expect(true).toBe(false);
            }, function(reason) {
                expect(reason).toEqual("primary rejected");
            });

        return promise;
    });

    it("accepts progress callback for secondary promise notifications", function () {
        var primaryResolved = Q(1);
        var secondaryDeferred = Q.defer();
        var progress = false;

        promise = primaryResolved
            .after(secondaryDeferred.promise, function() {
                progress = true;
            })
            .then(function() {
        ...
        secondaryDeferred.notify();
        secondaryDeferred.resolve();
        return promise;
    });
});
