...
let REASON = "this is not an error, but it might show up in the console";
...
describe("computing sum of integers using promises", function() {
    it("should compute correct result without blowing stack", function () {
        ...
        let pZero = Q.fulfill(0);
        let result = array.reduce(function (promise, nextVal) {
...
describe("Q function", function () {
    ...
    it("should be the identity when given promise", function () {
        let f = Q.fulfill(5);
        let r = Q.reject(new Error("aaargh"));
        let p = Q.promise(function () { });
...
describe("defer and when", function () {
    ...
    it("resolve before when", function () {
        ...
        deferred.resolve(10);
        let promise = Q.when(deferred.promise, function (value) {
    ...
    it("reject before when", function () {
        ...
        let deferred = Q.defer();
        deferred.reject(-1);
        let promise = Q.when(deferred.promise, function () {
    ...
    it("when before resolve", function () {
        let turn = 0;
        let deferred = Q.defer();
        let promise = deferred.promise.then(function (value) {
    ...
    it("when before reject", function () {
        let turn = 0;
        let deferred = Q.defer();
        let promise = deferred.promise.then(function () {
            expect(true).toBe(false);
        }, function (value) {
            expect(turn).toEqual(2);
            expect(value).toEqual(-1);
    ...
    it("resolves multiple observers", function (done) {
        ...
        let resolution = "Taram pam param!";
        let deferred = Q.defer();
        let count = 10;
        let i = 0;
...
describe("always next tick", function () {
    ...
    it("generated by `reject`", function () {
        let turn = 0;
        let promise = Q.when(Q.reject(), function () {
            expect(true).toBe(false);
        }, function () {
            expect(turn).toEqual(1);
        });
...
describe("progress", function () {
    ...
    it("calls the progress listener with the notify values", function () {
        let progressValues = [];
        let desiredProgressValues = [{}, {}, "foo", 5];
        let deferred = Q.defer();
        let promise = Q.when(
            deferred.promise,
            function () {
                for (let i = 0; i < desiredProgressValues.length; ++i) {
                    let desired = desiredProgressValues[i];
                    let actual = progressValues[i];
